...
foreach ($data_absensi as $key => $value){ ?>
    <?php if ($iter == 0 OR $iter % 26 == 0){ ?>
        ...
        <?php if ($iter == 0){ ?>
            <br>
            <h1>Laporan Absensi Anak&nbsp;a.n&nbsp;<span class="text-success font-weight-bold"><?= $data_anak->nama_anak ?></span><br>Usia:&nbsp;<span style="color: green"><?= hitung_usia($data_anak->tanggal_lahir) ?> <span style="color: grey"><i>(<?= $data_anak->nama_kelas ?>)</i></span></span></h1>
        <?php } ?>
        <br>
    ...
    <tr>
        ...
        <td nowrap>
            <?php if (!empty($value->id_absensi)) { ?>
                &bullet;&nbsp;<span class="text-muted font-italic" style="font-size: 10px; color: grey; font-style: italic">Suhu Tubuh: <b><?= $value->suhu ?> °C</b>, Kondisi: <b><?= $value->kondisi == 1 ? 'Sehat':'Kurang Sehat' ?></b></span>
                <?php if (!empty($value->waktu_checkout)){ ?>
                    <br>
